...
const ImportantDashboard = () => {
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get("http://localhost:5000/api/importanttasks", {
    ...
    <div className='user-dashboard'>
      ...
      <div className="sidebar d-flex flex-column flex-shrink-0 p-3 text-dark" style={{width: '250px'}}>
        <ul className="nav nav-pills flex-column mb-auto">
          ...
          <li>
            <a onClick={toImportantDashboard} className="nav-link ">
              ...
              {tasks.pendingTasks.map((task, index) => (
                <div key={task._id} className="task">
                  ...
                  <div className="task_fotter">
                    ...
                    <button onClick={() =>  toUnImportant(task._id)}>
                     Unimportant
                    </button>
                  </div>
                </div>
